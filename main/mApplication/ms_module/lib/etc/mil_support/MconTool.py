...
def Exe(ev=None):
    ...
    if selobjchk==1:
        ...
        map( lambda i: Mcon(slls[i], slls[hSelSz+i], pvtCalc=pcchk, pos=0 if mochk else 1,t=tchk, r=rchk, s=schk, sh=shchk ), range( hSelSz ) )
    elif selobjchk==2:
        map( lambda i: Mcon(slls[0], slls[i], pvtCalc=pcchk, pos=0 if mochk else 1,t=tchk, r=rchk, s=schk, sh=shchk ) if i!=0 else None, range( len(slls) ) )
    pm.select(slls)

def Del(ev=None):
    map( lambda a: DelMcon(a), pm.ls(sl=1) )
