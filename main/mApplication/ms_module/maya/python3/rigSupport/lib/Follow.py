...
for i,item in enumerate(list_):
    ...
    poplist.pop(i)
    for p in poplist:
        pm.setDrivenKeyframe(const.attr('{0}W{1}'.format(spclist[i].name().split(':')[-1],i)),cd=sel[0].Follow,dv=p,v=0)
    pm.setDrivenKeyframe(const.attr('{0}W{1}'.format(spclist[i].name().split(':')[-1],i)),cd=sel[0].Follow,dv=i,v=1)
