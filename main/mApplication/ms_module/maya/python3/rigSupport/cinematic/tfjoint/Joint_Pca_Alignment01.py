...
# 1. 회전 비교 및 locator 생성
analyze_joint_face_rotation_aligned("joint1", "meshA", "meshB")

# 2. 평균 회전값 계산
locators = [f"loc_B_{d}" for d in ['x', 'y', 'z', '-x', '-y', '-z']]
pos, rot = compute_average_transform_from_locators(locators)

# 3. joint1의 rotate → 0, jointOrient → 새 회전값 적용
apply_joint_orient_with_rotate_axis("joint1", pos, rot)
